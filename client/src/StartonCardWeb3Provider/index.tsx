...
const CardSigning = () => {
	...
	const verifyWallet = async () => {
		...
		try {
			const sign = await library?.getSigner().signMessage("On n'interchange pas une equipe qui gagne" as string);
			/* Send the return of the signature at your back end here
			 await axios.post('/verify-message', ...)
				*/
			await axios.post("http://localhost:4242/verify-signature", {
				userSignature: sign
			});
			setVerified(true);
			router.push('/team-up')
